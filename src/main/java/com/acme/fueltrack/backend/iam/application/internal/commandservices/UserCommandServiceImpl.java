...
public class UserCommandServiceImpl implements UserCommandService {
    ...
    public Optional<User> handle(SignUpCommand command) {
        ...
        var roles = command.roles().stream().map(role -> profileRepository.findByRol(role.getRol()).orElseThrow(() -> new RuntimeException("Role not found"))).toList();
        var user = new User(
                command.username(),
                command.email(),
        ...
        return userRepository.findById(command.userId())
                .map(user -> {
                    ...
                    if (command.roles() != null && !command.roles().isEmpty()) {
                        ...
                        String role = command.roles().iterator().next();
                        Profile newProfile = profileRepository.findByRol(Roles.valueOf(role))
